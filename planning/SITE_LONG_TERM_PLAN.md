...
- 문서 상태: 사용자 승인 후 1차 구현 기준 명세
- 작성일: 2026-09-10
- 적용 범위: 1학기 CA 프로젝트 전용
- 구현 상태: **2026-09-14 1차 구현·파스텔 테마·브라우저 학습 진도까지 배포 완료**
...
| 학습 진도 | 12개 페이지 수동 완료, 홈 진행률·단계·체크, 퀴즈 누적 통계, 2단계 초기화 | `index.html`, `assets/js/progress.js` |
...
2026-09-14 사용자가 확정한 뇌맵 팔레트(분홍·라벤더·하늘·노랑·은색)를 사이트 전체의 시각 언어로 사용한다. 기본은 밝은 파스텔 배경이며, 같은 영역색을 유지하는 어두운 모드를 선택할 수 있게 한다. 카드형 아카이브 구조는 보존한다.
...
뇌맵은 홈 핵심 CTA와 모든 페이지의 상단 주 메뉴에서 바로 접근한다.

요구 동작:

- 현재 페이지는 파스텔 그라데이션 밑줄과 `aria-current="page"`로 표시한다.
...
- 구현 승인 전에는 HTML/CSS/JS를 수정하지 않는다. 2026-09-13의 명시적 구현 지시로 이 조건은 충족되었다.
...
## 19. 2026-09-13 1차 구현 결과

사용자의 명시적 구현 승인에 따라 다음 범위를 작성했다.

- canonical HTML 18개와 이전 주소 redirect 5개
- 공통 CSS 5개, 기능 JavaScript 6개
- 검색 색인 80항목, 퀴즈 45문항, 공개 기록 6개, 참고자료 15개
- 데이터 기반 홈, 모바일 햄버거, 접근 가능한 뇌맵, 연구 아카이브와 로컬 편집기
- 기존 `basic-neuroscience.html` 핵심 섹션 보존 및 전기생리·과학 검수 보강

정적 검증에서 필수 파일, 내부 링크·앵커, JSON, JavaScript 구문, 검색·퀴즈 연결, 가상 홈 수치 제거, PDF 미포함, 뇌맵 형태 변형 금지를 확인했다.

남은 검수는 로컬 실행 서버 복구 후 실제 브라우저에서 360·768·1024·1440px 레이아웃, 콘솔, 터치와 localStorage·파일 다운로드 동작을 재확인하는 것이다. 구현 자체는 승인된 상태이며, 이후에는 이 문서를 다시 ‘구현 금지’ 상태로 해석하지 않는다.

## 20. 2026-09-14 파스텔 테마·뇌맵 후속 결과

- `assets/css/theme.css`에 밝은 파스텔 기본 테마와 어두운 테마를 추가했다.
- Nunito와 Noto Sans KR을 제목·본문 역할에 맞게 적용했다.
- 테마 선택은 `neuro-archive-theme` 키로 현재 브라우저에 저장한다.
- 홈의 형광색 뇌 이미지를 원본 뇌맵 팔레트와 닮은 CSS 도식으로 교체했다.
- 상단 주 메뉴에 독립 `뇌맵` 링크를 추가했다.
- 소뇌의 확정된 외곽·방향은 유지하면서 라벨 flex 우선순위 오류만 수정했다.
- 모바일에서 뇌맵에 강제되던 `min-width:540px`을 제거했다.
- 공개 Pages 1363×936 화면에서 두 테마, hover 형태 고정, 툴팁 경계, Space 이동, 소뇌 라벨, 대표 페이지의 사이트 출처 콘솔 오류 0개를 확인했다.

최신 원격 `main`은 `32a78261d642ec78acc188684d45fb6a347e7617`이며 Pages 실행 `34801127985`가 성공했다. 남은 핵심 검수는 실제 360·768px 기기의 햄버거와 두 번 터치 흐름이다.

## 21. 2026-09-14 학습 진도·게이미피케이션 후속 결과

- 7개 핵심 학습 페이지와 5개 뇌영역 상세 페이지, 총 12개를 명시적 완료 대상으로 정했다.
- 단순 방문을 완료로 오인하지 않도록 각 페이지의 `학습 완료로 표시` 버튼을 눌러야 저장한다.
- 홈에 완료 수·진행 막대·단계명·페이지별 체크리스트와 퀴즈 도전·응답·정답률을 표시한다.
- 완료한 페이지로 향하는 추천 카드에는 `✓ 완료` 배지를 붙인다.
- 학습 진도는 `neuroArchive.learningProgress.v1`, 기존 퀴즈 누적과 세션은 각각 별도 키에 저장한다.
- 진도 초기화는 첫 질문에서 `예, 계속`을 누른 뒤 `정말 초기화할까요?`라는 두 번째 질문에서 최종 확인해야 실행된다.
- 초기화 대상은 학습 완료·퀴즈 누적·퀴즈 세션으로 제한한다. `neuroArchive.records.v1` 연구기록과 `neuro-archive-theme` 테마 설정은 건드리지 않는다.
- 공개 Pages에서 완료 표시, 새로고침 복구, 홈 체크·레벨 반영, 퀴즈 누적 통계, 초기화 두 단계·취소·Escape를 확인했다.

학습 진도 기능 기준 커밋은 `d0719b0fb809deda56ebcd0ae8a1a5f988ce30e2`이며 Pages 실행 `34819298417`이 성공했다. 이 문서 갱신을 위한 후속 체크포인트 커밋이 이어지므로 다음 작업자는 원격 `main`의 현재 SHA를 다시 확인한다. 남은 핵심 검수는 실제 360·768px 기기에서 진행도 체크리스트와 대화상자 간격을 확인하는 것이다.
